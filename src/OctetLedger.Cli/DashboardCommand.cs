...
</style></head><body><h1>OctetLedger</h1><div class="muted">Local network usage · last 30 days</div><div class="cards"><div class="card">30-day total<div class="value" id="total">—</div></div><div class="card">Daily average<div class="value" id="average">—</div></div><div class="card">Peak day<div class="value" id="peak">—</div></div></div><div class="chart" id="chart"></div><table><thead><tr><th>Day</th><th>Interface</th><th>Download</th><th>Upload</th><th>Total</th></tr></thead><tbody id="rows"></tbody></table><script>
const fmt=n=>{const u=['B','KiB','MiB','GiB','TiB'];let i=0;while(n>=1024&&i<u.length-1){n/=1024;i++}return n.toFixed(n>=100?0:n>=10?1:2)+' '+u[i]};
fetch('/api/data').then(r=>r.json()).then(({rows})=>{const value=(r,n)=>r[n]??r[n[0].toLowerCase()+n.slice(1)],daily=new Map;for(const r of rows){const period=value(r,'Period'),total=value(r,'TotalBytes');daily.set(period,(daily.get(period)??0)+total)}const days=[...daily],totals=days.map(([,total])=>total),sum=totals.reduce((a,b)=>a+b,0),max=Math.max(1,...totals);total.textContent=fmt(sum);average.textContent=fmt(sum/Math.max(1,days.length));peak.textContent=fmt(Math.max(0,...totals));chart.innerHTML=days.map(([period,total])=>`<div class="bar" title="${period}: ${fmt(total)}" style="height:${total/max*100}%"></div>`).join('');document.querySelector('#rows').innerHTML=[...rows].reverse().map(r=>`<tr><td>${value(r,'Period')}</td><td>${value(r,'InterfaceName')}</td><td>${fmt(value(r,'BytesReceived'))}</td><td>${fmt(value(r,'BytesSent'))}</td><td>${fmt(value(r,'TotalBytes'))}</td></tr>`).join('')});
</script></body></html>
""";
}
